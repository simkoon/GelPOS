let Joi,User,Transporter;_58c‍.x([["code",()=>code],["findCode",()=>findCode],["register",()=>register],["login",()=>login],["check",()=>check],["logout",()=>logout],["pwChange",()=>pwChange]]);_58c‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_58c‍.w("../../models/user",[["default",["User"],function(v){User=v}]]);_58c‍.w("./Email/registMail",[["default",["Transporter"],function(v){Transporter=v}]]);
...
// 아이디 비밀변호 변경할때 코드
       const findCode = async (ctx) => {
  const { email } = ctx.request.body;

  try {
    const existsEmail = await User.findByEmail(email);
    _58c‍.g.console.log(existsEmail);
    if (!existsEmail) {
      ctx.body = {
        emailoverlap: true,
      };
  ...
  // Request Body 검증하기
  _58c‍.g.console.log('body', ctx.request.body);
